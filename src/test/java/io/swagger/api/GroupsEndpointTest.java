...
public class GroupsEndpointTest {
	...
	/** The api groups endpoint url. */
	private final String apiGroupsEndpoint = "/groups";
	...
	/**
     * Check groups endpoint by non-permitted http-methods.
     *
    ...
    public void groupsEndpointNotPermittedHttpMethod() throws Exception {
    	...
    	filter.setFunctionIds(searchFunctions);
    		 ...
    		 * Put-Request-Test
    		 */
			MvcResult result = this.mvc.perform(put(apiGroupsEndpoint)
			 ...
			 * Get-Request-Test
			 */
			MvcResult result2 = this.mvc.perform(get(apiGroupsEndpoint)
					.contentType(MediaType.APPLICATION_JSON)
					.content(asJsonString(filter)))
					.andExpect(status().isMethodNotAllowed())
					.andReturn();
			 ...
			 * Head-Request-Test
			 */
			MvcResult result3 = this.mvc.perform(head(apiGroupsEndpoint)
					.contentType(MediaType.APPLICATION_JSON)
					.content(asJsonString(filter)))
					.andExpect(status().isMethodNotAllowed())
					.andReturn();
			 ...
			 * Delete-Request-Test
			 */
			MvcResult result4 = this.mvc.perform(delete(apiGroupsEndpoint)
					.contentType(MediaType.APPLICATION_JSON)
					.content(asJsonString(filter)))
					.andExpect(status().isMethodNotAllowed())
					.andReturn();
	...
	/**
	 * Get groups by non-json request.
	 *
	 * @throws Exception
	 *             the exception
	 */
     @Test
	 public void groupsPostEndpointNonJsonRequest() throws Exception {
	    	List <String> searchRooms = Arrays.asList("testroom_0");
	    	List <String> searchFunctions = Arrays.asList("testfunction_0");
			...
			MvcResult result3 = this.mvc.perform(post(apiGroupsEndpoint)
