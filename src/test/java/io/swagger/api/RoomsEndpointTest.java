...
public class RoomsEndpointTest {
	...
	private final String apiRoomEndpoint = "/rooms";
	...
	/**
     * Check rooms endpoint by non-permitted http-methods.
     *
    ...
    public void roomsEndpointNotPermittedHttpMethod() throws Exception {
    	...
    	filter.setFunctionIds(searchFunctions);
    		 ...
    		 * Put-Request-Test
    		 */
			MvcResult result = this.mvc.perform(put(apiRoomEndpoint)
			 ...
			 * Get-Request-Test
			 */
			MvcResult result2 = this.mvc.perform(get(apiRoomEndpoint)
					.contentType(MediaType.APPLICATION_JSON)
					.content(asJsonString(filter)))
					.andExpect(status().isMethodNotAllowed())
					.andReturn();
			 ...
			 * Head-Request-Test
			 */
			MvcResult result3 = this.mvc.perform(head(apiRoomEndpoint)
					.contentType(MediaType.APPLICATION_JSON)
					.content(asJsonString(filter)))
					.andExpect(status().isMethodNotAllowed())
					.andReturn();
			 ...
			 * Delete-Request-Test
			 */
			MvcResult result4 = this.mvc.perform(delete(apiRoomEndpoint)
					.contentType(MediaType.APPLICATION_JSON)
					.content(asJsonString(filter)))
					.andExpect(status().isMethodNotAllowed())
					.andReturn();
	 ...
	 */
     @Test
	 public void roomsPostEndpointNonJsonRequest() throws Exception {
	    	List <String> searchRooms = Arrays.asList("testroom_0");
	    	List <String> searchFunctions = Arrays.asList("testfunction_0");
			...
			MvcResult result3 = this.mvc.perform(post(apiRoomEndpoint)
